...
export const saveEmbeddings = async (chunks, mongoId, userId, type, brainId) => {
    const batchSize = 5;
    const embeddings = [];
    for (let i = 0; i < chunks.length; i += batchSize) {
        console.log(`Starting batch ${i}`);
        const batch = chunks.slice(i, i + batchSize);
        const embed = await Promise.all(batch.map(chunk => generateEmbeddings(chunk)));
    ...
    const chunkIds = chunks.map((_, i) => `${mongoId}_chunk_${i}`);
    await contentModel.findByIdAndUpdate(mongoId, { chunkIds });
...
//# sourceMappingURL=saveEmbeds.js.map