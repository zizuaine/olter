...
export const executeOperation = async (intent, query, resolvedContent, chat, user) => {
    ...
    if (intent.operation === "summary") {
        ...
        ;
        return handleActionResponse("summary", context ?? "", contentIds, user, chat._id.toString());
    }
    if (intent.operation === "flashcard" || intent.operation === "quiz") {
        if (!context) {
            throw new Error("No relevant context found for this operation");
        }
        return handleActionResponse(intent.operation, context, contentIds, user, chat._id.toString());
    }
