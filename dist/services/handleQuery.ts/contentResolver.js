...
export const contentResolver = async (intent, chat, query, user) => {
    ...
    if (intent.target === "active") {
        if (intent.scope === "relevant") {
            ...
            const records = await pineconeIndex.fetch({ ids: chat.activeChunksIds });
            const combinedContent = Object.values(records.records)
                .map(rec => rec.metadata?.text)
                .filter((text) => Boolean(text))
                .join("\n\n---\n\n");
            const contentIds = chat.activeContentIds.map(id => id.toString());
            return {
        ...
        if (intent.scope === "full") {
            ...
            const records = await pineconeIndex.fetch({ ids: chunkIds });
            ...
            return {
                batches,
                contentIds: chat.activeContentIds.map(id => id.toString())
            };
        }
    }
    ;
    ...
    if (intent.target === "topic") {
        const { sources, context, matches } = await semanticSearch(userQuery, user, brainId);
        if (sources.length === 0) {
    ...
    if (intent.target === "specific") {
        const { sources, matches } = await semanticSearch(userQuery, user, brainId);
